...
Mat3 ori::rpyToRotMat(const Vec3& v) {
  Mat3 m = ori::coordinateRotation(ori::CoordinateAxis::X, v[0]) *
           ...
           ori::coordinateRotation(ori::CoordinateAxis::Z,
                                   v[2]);  //[todo] maybe error
  // Mat3 m = ori::coordinateRotation(ori::CoordinateAxis::Z, v[2]) *
  //                                   ori::coordinateRotation(ori::CoordinateAxis::Y,
  //                                   v[1]) *
  //                                   ori::coordinateRotation(ori::CoordinateAxis::X,
  //                                   v[0]);
  return m;
}
...
void ori::rpy_safety_check(Vec3& angle_des, Vec3 angle_act) {
  for (int i = 0; i < 3; ++i) {
